...
<template>
                  ...
                  <div class="mb-3" >
                    ...
                    <div style="display: flex;">
                      <img 
                        ...
                        :key="index" 
                        :src="index < newComment.star ? '/src/assets/images/chngstar.gif' : '/src/assets/images/star.gif'" 
                  ...
                  <div class="mb-3">
                    ...
                    <div>
                      <img 
                        v-for="(star, index) in 5" 
                        :key="index" 
                        :src="index < editComment.star ? '/src/assets/images/chngstar.gif' : '/src/assets/images/star.gif'" 
                  ...
                  <div class="ms-2">
                    ...
                    <div>
                      <img 
                        v-for="(star, index) in 5" 
                        :key="index" 
                        :src="index < comment.star ? '/src/assets/images/chngstar.gif' : '/src/assets/images/star.gif'" 
                        class="star" 
                      />
                    </div>
                  </div>
      ...
      axiosapi.get(`/comments/status/${this.userId}`)
        .then(response => {
          ...
          if (response.data && Array.isArray(response.data)) {
            ...
            this.canComment = response.data.some(order => order.status === 1 && order.productId >= 91 && order.productId <= 99);
            console.log("canComment成功", this.canComment); // 打印 canComment 的值
  
            // 打印符合条件的订单信息
            response.data.forEach(order => {
              if (order.status === 1 && order.productId >= 91 && order.productId <= 99) {
  ...
  //重新更新頁面
  fetchComments() {
    axiosapi.get(`/comments/products/${this.productId}/99`)
        .then(response => {
