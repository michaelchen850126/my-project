    ...
    axiosapi.get(`/comments/status/${this.userId}`)
      .then(response => {
        ...
        if (response.data && Array.isArray(response.data)) {
          ...
          this.canComment = response.data.some(order => order.status === 1 && order.productId >= 19 && order.productId <= 27);
          console.log("canComment成功", this.canComment); // 打印 canComment 的值

          // 打印符合条件的订单信息
          response.data.forEach(order => {
            if (order.status === 1 && order.productId >= 19 && order.productId <= 27) {
...
//重新更新頁面
fetchComments() {
  axiosapi.get(`/comments/products/${this.productId}/27`)
      .then(response => {
