    ...
    axiosapi.get(`/comments/status/${this.userId}`)
      .then(response => {
        ...
        if (response.data && Array.isArray(response.data)) {
          ...
          this.canComment = response.data.some(order => order.status === 1 && order.productId >= 1 && order.productId <= 9);
          console.log("canComment成功", this.canComment); // 打印 canComment 的值

          // 打印符合条件的订单信息
          response.data.forEach(order => {
            if (order.status === 1 && order.productId >= 1 && order.productId <= 9) {
...
//重新更新頁面
fetchComments() {
  axiosapi.get(`/comments/products/${this.productId}/9`)
      .then(response => {
...
// 編輯文件上傳處理邏輯
    ...
    console.log('Star rating clicked:', rating, type); // 添加日志来检查
    if (type === 'newComment') {
